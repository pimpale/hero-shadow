...
public class MainLoop extends JPanel implements MouseListener, KeyListener {
  ...
  private static void playerinterface() {
    ...
    if (mousex < 700 && mousedown == true && gamepaused == false) {
      ...
      for (int i = 0; i < Entities.size(); i++) {
        ...
        if ((mousex + playerentity.X() - 350 >= bob.X() - playerentity.X()
                && mousex - playerentity.X() - 350 <= bob.X() + bob.Xdimension())
            && (mousey - playerentity.Y() - 350 > bob.Y()
                && mousey - playerentity.X() - 350 < bob.Y() + bob.Ydimension())) {
          mousex = 0;
          mousey = 0; // TODO
          mousedown = false;
          bob.Select();
      ...
      if (projectile.Type() == "melee") {
        Image meleestrikeup =
            Toolkit.getDefaultToolkit()
                .getImage("projectiles/meleestrike/range1 melee strike up.png");
        Image meleestrikedown =
            Toolkit.getDefaultToolkit()
                .getImage("projectiles/meleestrike/range1 melee strike down.png");
        Image meleestrikeleft =
            Toolkit.getDefaultToolkit()
                .getImage("projectiles/meleestrike/range1 melee strike left.png");
        // Image meleestrikeright =
        // Toolkit.getDefaultToolkit().getImage("projectiles/meleestrike/range1 melee strike
        // right.png");
        double xmin;
        double ymin;
        double xmax;
          ...
          if (!bob.Type().equals(projectile.Caster.Type())) {
            if (bob.X() > xmin && bob.X() < xmax && bob.Y() > ymin && bob.Y() < ymax) {
              ...
              for (int q = 0; q < Effects.length; q++) {
                ...
                bob.EffectWith(
                    parts[0] + "#" + projectile.Caster.X() + "#" + projectile.Caster.Y(),
  ...
  private void entity() {
    ...
    for (int i = 0; i < Entities.size(); i++) {
      ...
      double distance =
          Math.sqrt(
              Math.pow(playerentity.Y() - bob.Y(), 2) + Math.pow(playerentity.Y() - bob.X(), 2));
      if (bob.Active() == true) {
        bob.Update();
          ...
          for (int a = 0; a < xStart.size(); a++) {
            ...
            if ((iXmin < aXmax) && (iXmax > aXmin) && (iYmin < aYmax) && (iYmax > aYmin)) {
              String effect =
                  "knockback"
                      + "#"
                      + (xStart.get(a).intValue() + TILESIZE / 2)
                      + "#"
                      + (yStart.get(a).intValue() + TILESIZE / 2);
    ...
    if (playerExists == true) {
        ...
        for (int a = 0; a < 14; a++) {
          try {
            ...
            if (img1 != null) {
              g2d.drawImage(
                  img1,
                  (xoftile) * TILESIZE - (int) playerentity.X() + 350,
                  (yoftile) * TILESIZE - (int) playerentity.Y() + 350,
                  this);
            }
          } catch (NullPointerException e) {
      ...
      for (int i = 0; i < Entities.size(); i++) {
        ...
        if (bob.Active()) {
          g2d.drawImage(
              bob.Sprite,
              (int) (bob.X() - playerentity.X + 350),
              (int) (bob.Y() - playerentity.Y + 350),
              null);
          // g2d.drawRect((int)(bob.X()-playerentity.X+350), (int)(bob.Y()-playerentity.Y+350),
          // bob.Xdimension(), bob.Ydimension());
        }
      }
      ...
      if (InventoryOn == false) {
            ...
            if (draggedXpos != i || draggedYpos != a) {
              ...
              if (thing != null) {
                ...
                if (thing.count > 1) {
                  g2d.setPaint(Color.WHITE);
                  g2d.drawString(
                      Integer.toString(thing.count), 100 + (i) * 50 + 40, 100 + (a) * 50 + 18);
                }
              }
            }
          }
        ...
        if (draggedXpos != -1 && draggedXpos != -1) {
          if (draggedXpos < 10) {
            if (Item1[draggedYpos][draggedXpos] != null)
              g2d.drawImage(
                  Item1[draggedYpos][draggedXpos].sprite,
                  (int) mousex - 25,
                  (int) mousey - 25,
                  null);
          } else if (draggedXpos == 10) {
            Item[] Item2 = UsefulMethods.DisplayHotbar();
            if (Item2[draggedYpos] != null)
              g2d.drawImage(Item2[draggedYpos].sprite, (int) mousex - 25, (int) mousey - 25, null);
          } else if (draggedXpos == 11 && draggedYpos == 0) {
            Item[] Item3 = UsefulMethods.DisplayArmor();
            if (Item3[draggedYpos] != null)
              g2d.drawImage(Item3[draggedYpos].sprite, (int) mousex - 25, (int) mousey - 25, null);
          } else {
            Item[] Item4 = UsefulMethods.DisplayDrops();
            if (Item4[draggedYpos - 1] != null)
              g2d.drawImage(
                  Item4[draggedYpos - 1].sprite, (int) mousex - 25, (int) mousey - 25, null);
          }
        }
      }
